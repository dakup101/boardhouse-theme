...
$img          = wp_get_attachment_image( $img_id, 'large', '', array( "class" => "absolute lg:h-full lg:w-auto min-w-full min-h-full object-cover top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10" ) );
...
<div class="product-card border-light-gray relative border flex flex-col h-full">
    ...
    <div class="px-5 pt-3 pb-5 flex h-full flex-col justify-between">
        ...
        <span
            class="flex flex-col sm:flex-row tracking-wider px-5 font-bold mt-5 text-sm text-dark-gray <?php echo ! $is_variable  ? 'justify-center sm:justify-between' : 'justify-center' ?> justify-between"><?php echo $price_string; ?></span>
    </div>
</div>